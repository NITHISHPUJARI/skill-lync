...
// Number of callback registerd:
static int NumCallBackReg = 0;

// Timer callback Function pointer array:
#define MAX_TIMER_CALLBACK  5
void (*timer_callback_arr[MAX_TIMER_CALLBACK])();
...
void Register_TimerCallback(void (*callback)())
{

    if (NumCallBackReg < MAX_TIMER_CALLBACK)
    {
        serial_puts("\r\nNumCallBackReg = ");
        serial_put_uint(NumCallBackReg);
        (timer_callback_arr[NumCallBackReg]) = callback;
        NumCallBackReg += 1;
    }
    else
    {
        serial_puts("\r\nNumCallBackReg = ");
        serial_put_uint(NumCallBackReg);
        serial_puts("\r\nCant Reg New Callbacks!!\r\n");
    }
}

void dispatch_timer_callbacks(){

    for(int i = 0; i < NumCallBackReg; i++){
        (*timer_callback_arr[i])();
    }
}
