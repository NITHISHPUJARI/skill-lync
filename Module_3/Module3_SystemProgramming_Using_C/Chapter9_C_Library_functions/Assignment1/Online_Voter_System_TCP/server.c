...
void *communicate (void *message)
{
	...
	//--------------------------------------------------------------------------//	
	// Take name and EPIC  from client

	//Receive Name/EPIC  from the client
	...
	// Read Voters_List.txt and match EPIC  and name, if match_exists then allow to vote else end thread
	FILE *fp;
	char buff[255];
	...
	if (voter_found == 1)
	{
		...
		if (vote_casted == 0)
		{
			...
			while (fgets(buff, sizeof(buff), fp) != NULL)
			{
				...
				snprintf((candidates + strlen(candidates)), strlen(candidates), "%s", buff);
			}
			...
			// strncpy(server_message, candidates);
			snprintf(server_message, strlen(candidates), "%s", candidates);
